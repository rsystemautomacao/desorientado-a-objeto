...
function parseServiceAccountEnv(): Record<string, unknown> {
  ...
  if (!envJson || typeof envJson !== 'string') throw new Error('Nenhuma service account configurada.');
  try {
...
function ensureFirebaseApp() {
  ...
  const projectId = (raw.project_id ?? raw.projectId ?? 'desorientado-a-objetos') as string;
