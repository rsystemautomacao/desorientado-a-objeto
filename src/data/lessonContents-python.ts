...
export const pythonLessonContents: Record<string, LessonContent> = {
  ...
  'py-m1-intro': {
    ...
    sections: [
      ...
      {
        title: 'Indentação — A Regra de Ouro do Python',
        body: 'Em Python, a indentação (os espaços no início da linha) define os blocos de código — o papel que chaves {} fazem em C ou Java.\n\nCada nível de bloco usa 4 espaços a mais. Não há chaves, ponto e vírgula obrigatório, nem begin/end — apenas a indentação.',
        code: '# CORRETO — blocos indentados com 4 espaços\nif True:\n    print("Este bloco está indentado")  # 4 espaços\n    print("Ainda dentro do bloco")      # 4 espaços\nprint("Fora do bloco")                  # sem indentação\n\n# ERRADO — falta indentação\nif True:\nprint("Isso gera IndentationError!")',
        codeExplanation: 'A falta de indentação após o if gera um IndentationError — o programa não executa. É um dos erros mais comuns em Python para iniciantes.',
        warning: 'IndentationError significa que o Python encontrou um bloco sem a indentação esperada. Use sempre 4 espaços (ou 1 tab, mas nunca misture os dois no mesmo arquivo). Editores como VS Code e PyCharm indendam automaticamente ao pressionar Enter após os dois-pontos (:).',
      },
    ],
    summary: [
      ...
      'Indentação (4 espaços) define blocos de código — obrigatório!',
      'Falta ou excesso de indentação gera IndentationError',
    ],
  },
  ...
  'py-m2-ifelse': {
    ...
    sections: [
      ...
      {
        title: 'Valores Falsy e Comparações Encadeadas',
        body: 'Em Python, alguns valores são considerados falsos (falsy) em contextos booleanos — não é só False que é falso!\n\nSão falsy: False, None, 0, 0.0, "" (string vazia), [] (lista vazia), {} (dicionário/set vazio).\n\nAlém disso, Python permite encadear comparações de forma natural:',
        code: '# Valores falsy — todos avaliam como False no if\nif 0:\n    print("Não executa — 0 é falsy")\nif "":\n    print("Não executa — string vazia é falsy")\nif []:\n    print("Não executa — lista vazia é falsy")\nif None:\n    print("Não executa — None é falsy")\n\n# Comparações encadeadas — idioma Python!\nx = 5\nif 1 <= x <= 10:           # equivale a: x >= 1 and x <= 10\n    print("x está entre 1 e 10")\n\nidade = 20\nif 18 <= idade < 65:\n    print("Adulto em idade ativa")',
        codeExplanation: 'Python permite encadear comparações: 1 <= x <= 10 é válido e idiomático. Equivale a x >= 1 and x <= 10, mas é mais legível. Use para verificar intervalos.',
        tip: 'Um pattern comum: if lista: (executa se a lista não for vazia) é preferível a if len(lista) > 0: — é mais pythônico e legível.',
      },
    ],
    summary: [
      ...
      'Valores falsy: False, None, 0, "", [], {} — avaliados como False no if',
      'Comparações encadeadas: 1 <= x <= 10 é válido e idiomático em Python',
    ],
  },
  ...
  'py-m3-functions': {
    ...
    sections: [
      ...
      {
        title: 'Funções Lambda (Anônimas)',
        body: 'Lambda cria funções anônimas (sem nome) de uma única expressão. São úteis quando você precisa de uma função pequena em um único lugar, sem precisar nomeá-la com def.',
        code: '# Função normal com def\ndef dobrar(x):\n    return x * 2\n\n# Equivalente com lambda\ndobrar = lambda x: x * 2\n\nprint(dobrar(5))   # 10\n\n# Lambda com múltiplos parâmetros\nsomar = lambda a, b: a + b\nprint(somar(3, 4)) # 7\n\n# Uso clássico: ordenar por critério personalizado\nalunos = [("Carlos", 7.5), ("Ana", 9.0), ("Bruno", 8.2)]\nalunos.sort(key=lambda a: a[1], reverse=True)  # ordena pela nota\nfor nome, nota in alunos:\n    print(f"{nome}: {nota}")',
        codeExplanation: 'lambda parâmetros: expressão — sem chaves, sem return. A expressão é o valor retornado. Em alunos.sort(key=...), a lambda recebe cada elemento e retorna o critério de ordenação.',
        tip: 'Use lambda para funções simples de uma linha, especialmente como argumento de sort(), sorted(), map() e filter(). Para lógica mais complexa, prefira def — o código fica mais legível.',
      },
    ],
    summary: [
      ...
      'lambda parâmetros: expressão — função anônima de uma linha',
    ],
  },
